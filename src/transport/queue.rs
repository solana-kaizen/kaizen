...
impl TransactionQueue {
    ...
    pub fn find_tx_chain_with_transaction(
        &self,
        tx: &Arc<Transaction>,
    ) -> Result<Option<Arc<TransactionChain>>> {
        let tx_id = tx.id;
    ...
    pub fn find_tx_chain_account_intersection(
        &self,
        ...
        log_trace!(
            "find_tx_chain_account_intersection: transaction:{:?}\n pending-chains len = {}",
            transaction,
            pending.len()
        );
        for (_, tx_chain) in pending.iter() {
            ...
            log_trace!(
                "find_tx_chain_account_intersection: chain accounts {:?}, tx_accounts:{:?}",
                accounts,
                tx_accounts
            );
    ...
    async fn enqueue_only(&self, transaction: Arc<Transaction>) -> Result<Arc<TransactionChain>> {
            ...
            let tx_chain = match locked {
                ...
                TransactionStatus::Timeout | TransactionStatus::Error(_) => {
                    if let Some(tx_chain) = queue.find_tx_chain_with_transaction(&transaction)? {
                        ...
                        log_warning!(
                            "Unable to find transaction chain during transaction resubmission"
                        );
                        None
                    }
                }
                ...
                TransactionStatus::Pending => {
                    ...
                    if let Some(tx_chain) =
                        queue.find_tx_chain_account_intersection(&transaction)?
                    {
                        tx_chain.extend_with(&[transaction.clone()])?;
                        for observer in queue.observers()?.iter() {
                            observer
                                .tx_created(tx_chain.clone(), transaction.clone())
                                .await;
                        }
                        Some(tx_chain)
                    } else {
        ...
        log_trace!(
            "find_tx_chain_account_intersection: id:{}, tx_chain.accounts(): {:?}",
            tx_chain.id,
            tx_chain.accounts()
        );
        Ok(tx_chain)
    ...
    pub async fn enqueue_multiple(&self, transactions: Vec<Arc<Transaction>>) -> Result<()> {
        ...
        let list: Vec<Arc<TransactionChain>> = chains.into_values().collect();
    ...
    async fn process_chains(&self, tx_chains: Vec<Arc<TransactionChain>>) -> Result<()> {
        for tx_chain in tx_chains {
            ...
            workflow_core::task::spawn(async move {
                log_trace!(
                    "find_tx_chain_account_intersection chain processing , id: {}",
                    tx_chain.id
                );
                match queue.process_transaction_chain_task(&tx_chain).await {
                    ...
                    Err(err) => {
                        ...
                        log_error!(
                            "TransactionQueue::process_transaction_task failure: {}",
                            err
                        );
                    }
                }
        ...
        loop {
            ...
            if let Some(tx) = tx {
                ...
                let result = match &tx.instruction {
                    Some(instruction) => transport.execute(instruction).await,
                    None => Ok(()),
                };
                match result {
                    Ok(_) => {
                    ...
                    Err(err) => {
                        ...
                        for observer in observers.iter() {
                            observer
                                .tx_failure(tx_chain.clone(), tx.clone(), err.clone())
                                .await;
                        }
