    ...
    T: Copy + 'info,
{
    ...
    pub fn get_meta(&self) -> &'info mut ArrayMeta {
        ArrayMeta::from_account_buffer_mut(self.account, self.get_offset())
    }
    ...
    pub fn try_init_meta_with_templates(&self, records: usize) -> Result<&mut ArrayMeta> {
    ...
    pub fn try_init_meta_with_records(&self, records: &[T]) -> Result<&'info mut ArrayMeta>
    where
        T: 'info + Copy,
    {
        #[cfg(feature = "check-buffer-sizes")]
    ...
    pub fn try_init_meta_with_refs(&self, records: &[&T]) -> Result<&mut ArrayMeta>
    where
        T: 'info + Copy,
    {
        #[cfg(feature = "check-buffer-sizes")]
    ...
    pub fn try_get_mut_at(&self, idx: usize) -> Result<&'refs mut T> {
        ...
        let data = self.account.data.borrow();
        Ok(unsafe {
            &mut *((data[(data_offset + idx * mem::size_of::<T>())..]).as_ptr() as *mut T)
        })
    }
    ...
    pub unsafe fn try_resize_for_items(&self, records: usize, zero_init: bool) -> Result<()> {
        ...
        log_trace!(
            "***########### resize for items -  capacity: {}  new_byte_len: {}",
            capacity,
            new_byte_len
        );
        // panic!("***");
...
// where T : 'info
    ...
    pub fn try_remove_at_swap_last(
        &self,
        idx: usize,
        realloc: bool,
        zero_init: bool,
        ...
        if realloc {
            ...
            if zero_init {
                utils::fill_account_buffer_u8(
                    self.account,
                    new_len..new_len + mem::size_of::<T>(),
                    0,
                );
            }
...
pub struct MappedArrayIterator<'info, T>
where
    T: 'info,
{
    idx: usize,
