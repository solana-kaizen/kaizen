...
impl<'info, 'refs> Segment<'info, 'refs> {
    ...
    pub fn try_resize(&self, new_data_len: usize, zero_init: bool) -> Result<()> {
        if self.resizable {
            self.store
                .try_resize_segment(self.idx, new_data_len, zero_init)
        } else {
    ...
    pub fn try_as_struct_ref<T>(&self) -> Result<&T>
    where
        T: 'info,
    {
...
impl<T> Clone for IndexEntry<T>
where
    T: Integer + IndexUnit + Copy,
{
...
impl<T> Layout<T>
where
    T: Debug + Integer + IndexUnit + Copy,
{
...
impl<'info, 'refs> SegmentStore<'info, 'refs> {
    ...
    pub fn try_create<T>(
        ...
        offset: usize,
        layout: &Layout<T>, // layout : &SegmentLayout
    ) -> Result<SegmentStore<'info, 'refs>>
    where
        T: 'info + Integer + Debug + IndexUnit + Copy,
    {
        ...
        #[cfg(feature = "check-buffer-sizes")]
        {
            ...
            if account.data_len() < data_len_needed {
                ...
                log_trace!(
                    "data_len_needed: {}  account.data_len(): {}",
                    data_len_needed,
                    account.data_len()
                );
                log_trace!("\n-------------------");
    ...
    pub fn try_load(
        ...
        if magic != SEGMENT_STORE_MAGIC {
            log_trace!(
                "[{}] segment store magic {:#x} should be: {:#x}",
                account.key,
                magic,
                SEGMENT_STORE_MAGIC
            );
            ...
            return Err(error_code!(ErrorCode::SegmentStoreMagic).with_account(account.key));
        }
    ...
    pub fn try_purge_segment(&mut self, idx: usize) -> Result<()> {
        match self.index_unit_size {
            IndexUnitSize::Bits16 => Ok(self.try_purge_segment_impl::<u16>(idx)?),
            IndexUnitSize::Bits32 => Ok(self.try_purge_segment_impl::<u32>(idx)?),
        }
    }

    pub fn try_purge_segment_impl<T>(&mut self, idx: usize) -> Result<()>
    where
        T: 'info + Integer + IndexUnit + Copy,
    {
        ...
        account_info_realloc(
            self.account,
            account_data_len - segment_data_len,
            false,
            false,
        )?;
    ...
    pub fn try_resize_segment(&self, idx: usize, new_len: usize, zero_init: bool) -> Result<()> {
        match self.index_unit_size {
            ...
            IndexUnitSize::Bits32 => self.try_resize_segment_impl::<u32>(idx, new_len, zero_init),
        }
    }

    pub fn try_resize_segment_impl<T>(
        &self,
        idx: usize,
        new_len: usize,
        zero_init: bool,
    ...
    pub fn get_index<'f, T>(&'f self) -> &'f mut [IndexEntry<T>]
    where
        T: 'info + Integer + IndexUnit,
    {
    ...
    pub fn get_index_at<'f, T>(&'f self, idx: usize) -> &'f IndexEntry<T>
    where
        T: 'info + Integer + IndexUnit,
    {
        let index = self.get_index::<T>();
    ...
    pub fn try_get_segment_ref_u8(&self, idx: usize) -> Result<&[u8]> {
        let segment = self.get_index_info_at(idx);
        Ok(utils::account_buffer_as_slice::<u8>(
            self.account,
            segment.offset,
            segment.size,
        ))
    ...
    pub fn try_get_segment_ref_mut_u8(&self, idx: usize) -> Result<&'info mut [u8]> {
        let segment = self.get_index_info_at(idx);
        Ok(utils::account_buffer_as_slice_mut::<u8>(
            self.account,
            segment.offset,
            segment.size,
        ))
    ...
    pub fn try_init_meta<T>(&self, new_index: &[IndexEntry<T>]) -> Result<()>
    where
        T: 'info + Integer + IndexUnit + Copy + std::fmt::Debug,
    {
...
mod tests {
    ...
    fn allocator_segment_test() -> Result<()> {
        ...
        unsafe {
            *(list_c.try_allocate(false)?) = 10u8;
        }
        ...
        // }
        unsafe {
            *(list_a.try_allocate(false)?) = 177u8;
        }
        // {
        //     let data = store.account.data.borrow();
